		...
		for i in xrange(dim_d):
			...
			with tf.name_scope('cellstate_%i' %i):
				...
				for ii in xrange(dim_t):
					...
					model['hc_%i_%i' %(i, ii)] = tf.where(tf.equal(model['hxi_%i' %ii], tf.zeros([dim_b], tf.int32)), model['hcc_%i_%i' %(i, ii)], tf.add(tf.multiply(model['hf_%i_%i' %(i, ii)], model['hcc_%i_%i' %(i, ii)]), tf.multiply(model['hi_%i_%i' %(i, ii)], tf.nn.tanh(tf.add(tf.matmul(model['hx_%i_%i' %(i, ii)], model['hWc_%i' %i]), model['hBc_%i' %i])))), name = 'hc_%i_%i' %(i, ii))
			...
			with tf.name_scope('attention_%i' %i):
				...
				for ii in xrange(dim_t):
					model['hat_%i_%i' %(i, ii)] = tf.nn.softmax(tf.add(tf.reshape(tf.transpose(tf.matmul(model['hm_%i' %i], tf.reshape(tf.transpose(tf.matmul(model['hWa_%i' %i], tf.transpose(model['hc_%i_%i' %(i, ii)]))), [dim_b, dim_i, 1]))), [dim_t, dim_b]), model['hBa_%i' %i]), name = 'hat_%i_%i' %(i, ii))
					model['hcx_%i_%i' %(i, ii)] = tf.reshape(tf.matmul(tf.reshape(tf.transpose(model['hat_%i_%i' %(i, ii)]), [dim_b, 1, dim_t]), model['hm_%i' %i]), [dim_b, dim_i], name = 'hcx_%i_%i' %(i, ii))
...
#
#		path_model = 'attention_model'
#		path_logdir = 'attention_logs'
#		saver = tf.train.Saver()
#		saver.save(sess, path_model)
#		writer = tf.summary.FileWriter(path_logdir, sess.graph)
