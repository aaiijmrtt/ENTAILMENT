...
def create(model, config):
	dim_v, dim_i, dim_d, dim_t, dim_m, dim_b, dim_n, dim_c = config.getint('vocabsize'), config.getint('wvecsize'), config.getint('depth'), config.getint('steps'), config.getint('memory'), config.getint('batch'), config.getint('deepness'), config.getint('classes')
		...
		for i in xrange(dim_d):
			...
			with tf.name_scope('cellstate_%i' %i):
				...
				for ii in xrange(dim_t):
					...
					model['pc_%i_%i' %(i, ii)] = tf.select(tf.equal(model['pxi_%i' %ii], tf.zeros([dim_b], tf.int32)), model['pcc_%i_%i' %(i, ii)], tf.add(tf.mul(model['pf_%i_%i' %(i, ii)], model['pcc_%i_%i' %(i, ii)]), tf.mul(model['pi_%i_%i' %(i, ii)], tf.nn.tanh(tf.add(tf.matmul(model['px_%i_%i' %(i, ii)], model['pWc_%i' %i]), model['pBc_%i' %i])))), name = 'pc_%i_%i' %(i, ii))
	...
	with tf.name_scope('memory'):
		for i in xrange(dim_d):
			model['hmi_%i' %i] = tf.reshape([model['pc_%i_%i' %(i, ii)] for ii in xrange(dim_t - dim_m, dim_t)], [dim_t, dim_b, dim_i], name = 'hmi_%i' %i)
			model['hm_%i' %i] = tf.transpose(model['hmi_%i' %i], [1, 0, 2], name = 'hm_%i' %i)
	...
	with tf.name_scope('hlstm'):
		...
		for i in xrange(dim_d):
			...
			with tf.name_scope('cellstate_%i' %i):
				...
				for ii in xrange(dim_t):
					model['hcc_%i_%i' %(i, ii)] = model['pc_%i_%i' %(i, dim_t - 1)] if ii == 0 else model['hc_%i_%i' %(i, ii - 1)]
					model['hc_%i_%i' %(i, ii)] = tf.select(tf.equal(model['hxi_%i' %ii], tf.zeros([dim_b], tf.int32)), model['hcc_%i_%i' %(i, ii)], tf.add(tf.mul(model['hf_%i_%i' %(i, ii)], model['hcc_%i_%i' %(i, ii)]), tf.mul(model['hi_%i_%i' %(i, ii)], tf.nn.tanh(tf.add(tf.matmul(model['hx_%i_%i' %(i, ii)], model['hWc_%i' %i]), model['hBc_%i' %i])))), name = 'hc_%i_%i' %(i, ii))

			with tf.name_scope('attention_%i' %i):
				model['hWa_%i' %i] = tf.Variable(tf.truncated_normal([dim_i, dim_i], stddev = 1.0 / dim_i), name = 'hWa_%i' %i)
				model['hBa_%i' %i] = tf.Variable(tf.truncated_normal([dim_t, 1], stddev = 1.0 / dim_t), name = 'hBa_%i' %i)
				for ii in xrange(dim_t):
					model['hat_%i_%i' %(i, ii)] = tf.nn.softmax(tf.add(tf.reshape(tf.transpose(tf.batch_matmul(model['hm_%i' %i], tf.reshape(tf.transpose(tf.matmul(model['hWa_%i' %i], tf.transpose(model['hc_%i_%i' %(i, ii)]))), [dim_b, dim_i, 1]))), [dim_t, dim_b]), model['hBa_%i' %i]), name = 'hat_%i_%i' %(i, ii))
					model['hcx_%i_%i' %(i, ii)] = tf.reshape(tf.batch_matmul(tf.reshape(tf.transpose(model['hat_%i_%i' %(i, ii)]), [dim_b, 1, dim_t]), model['hm_%i' %i]), [dim_b, dim_i], name = 'hcx_%i_%i' %(i, ii))

			with tf.name_scope('hidden_%i' %i):
				model['hWx_%i' %i] = tf.Variable(tf.truncated_normal([dim_i, dim_i], stddev = 1.0 / dim_i), name = 'hWx_%i' %i)
				...
				for ii in xrange(dim_t):
					model['hz_%i_%i' %(i, ii)] = tf.add(tf.add(tf.matmul(model['hcx_%i_%i' %(i, ii)], model['hWx_%i' %i]), tf.matmul(model['hc_%i_%i' %(i, ii)], model['hWz_%i' %i])), model['hBz_%i' %i], name = 'hz_%i_%i' %(i, ii))
